...
const updateStocks = () => ApiConnector.getStocks((response) => showStocks(response));
updateStocks();
...
const addMoneyHandler = (response) => {
    ...
    moneyManagerInstanse.setMessage(response.success, response.error || 'Успех!');
};
...
const showFavorites = (response, showMessage) => {
    ...
    if (showMessage) {
        favoritesWidgetInstanse.setMessage(response.success, response.error || 'Успех!')
    }
};
...
favoritesWidgetInstanse.addUserCallback = (data) => {
    ApiConnector.addUserToFavorites(data, (response) => showFavorites(response, true));
};

favoritesWidgetInstanse.removeUserCallback = (data) => {
    ApiConnector.removeUserFromFavorites(data, (response) => showFavorites(response, true));
};
